...
public class JoystickView extends View {
    ...
    private boolean disableAutoReturnToCenterY = false;
    private boolean disableAutoReturnToCenterX = false;
    ...
    public void setDisableAutoReturnToCenterX(boolean disableAutoReturnToCenterX) {
        this.disableAutoReturnToCenterX = disableAutoReturnToCenterX;
    }

    public boolean isDisableAutoReturnToCenterX() {
        return disableAutoReturnToCenterX;
    }

    public void setDisableAutoReturnToCenterY(boolean disableAutoReturnToCenterY) {
        this.disableAutoReturnToCenterY = disableAutoReturnToCenterY;
    }

    public boolean isDisableAutoReturnToCenterY() {
        return disableAutoReturnToCenterY;
    }
...
//              Log.d(TAG, String.format("ACTION_MOVE: (%03.0f, %03.0f) => (%03.0f, %03.0f)", x, y, touchX, touchY));
    ...
    private void reportOnMoved(boolean ignoreX, boolean ignoreY) {
        if ( movementConstraint == CONSTRAIN_CIRCLE )
            constrainCircle();
        else
            constrainBox();

        calcUserCoordinates(ignoreX, ignoreY);
...
//                              Log.d(TAG, String.format("moveListener.OnMoved(%d,%d)", (int)userX, (int)userY));
    ...
    // TODO fix differential coords
    private void calcUserCoordinates(boolean ignoreX, boolean ignoreY) {
...
//                              Log.d(TAG, "click");
    ...
    private void returnHandleToCenter() {
        if ( autoReturnToCenter && (!disableAutoReturnToCenterX || !disableAutoReturnToCenterY)) {
            ...
            for (int i = 0; i < numberOfFrames; i++) {
                ...
                postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        if (!disableAutoReturnToCenterX)
                        touchX += intervalsX;

                        if (!disableAutoReturnToCenterY)
                        touchY += intervalsY;

                        reportOnMoved(disableAutoReturnToCenterX, disableAutoReturnToCenterY);
                        invalidate();
